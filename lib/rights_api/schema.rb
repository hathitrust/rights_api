...
module RightsAPI
  class Schema
    ...
    # In cases where the "visible" key does not match the column name
    # (e.g., attribute vs attr) this is the visible (expanded) version.
    # @return [Symbol]
    def self.primary_key
      :id
    end

    # Transform an API parameter key into a Sequel expression
    # that can be used in a SQL WHERE or ORDER BY.
    # This is the default behavior typically called by subclasses after
    # any table-specific processing.
    #
    # Subclasses should sanity-check keys and raise an exception that is more
    # useful than a generic Sequel error.
    #
    # @param key [Symbol]
